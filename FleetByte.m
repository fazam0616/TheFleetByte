...
while(idx<=secs)               %% Main simulation loop
 ...
 length = size(HRS,2);
 rad = 5;
 HRSf = filter((1/rad)*ones(1,rad),1, HRS(:)); % Filter out noise from data
 filterStart = max(find(HRSf == 0,1,'last'),0);
 HRSf = HRSf(filterStart:size(HRSf,1),:) %Trim out 0 from beginning

 HRSdiff = diff(HRSf, 1, 1);
 HRSdiff = filter(ones(1,rad)/rad, 1, HRSdiff); %Smooth derivative
 avg = mean(HRSf); %Find baseline value to filter out secondary tremors (saddle points that follow peaks)
 indices = intersect(find(diff(sign(HRSdiff))<0) ,find(HRSf>avg));  %Peak detection, filters out secondary
                                                                    %tremors by comparing saddlepoints to avg
                                                                        
 
 if sum(HRS) ~= 0 %Sometimes sensor craps out all 0
     delHR = mean(diff(indices)); %Average time inbetween heart beats in device units
     hr=30/(10*delHR/length);
 else
     if idx == 1
         hr = 70 %BS Number in case sensor sends full 0 on frame 1
     end
 end
 ...
 % plot(HRSdiff, '-')
 % figure(7)
 % plot(diff(sign(HRSdiff)), '-')
 % figure(8);

 

 xyz=[MPS(1) MPS(2) MPS(3)];       % Replace with your computation of position, the map is 512x512 pixels in size
 ...
 delta=(xyz - prev);               % Replace with your computation for running direction, this should be a 2D unit vector
